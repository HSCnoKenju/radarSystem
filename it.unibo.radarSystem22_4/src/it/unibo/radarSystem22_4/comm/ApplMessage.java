...
public class ApplMessage implements IApplMessage {
    ...
    public void setConn(Interaction2021 conn) {
        if (isRequest())
            ...
            ColorsOut.out("WARNING: setting conn in a non-request message");
    }
    ...
    public String toString() {
        return "msg($msgId,$msgType,$msgSender,$msgReceiver,$msgContent,$msgNum)".replace("$msgId", msgId)
                .replace("$msgType", msgType).replace("$msgSender", msgSender).replace("$msgReceiver", msgReceiver)
